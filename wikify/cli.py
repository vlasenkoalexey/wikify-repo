...
def _host_instruction_block(wiki_dir: str = "wiki/code") -> str:
    """The retrieval block for a HOST wiki (many silos under ``<wiki_dir>/<slug>/``)."""
    w = wiki_dir.rstrip("/")
    return f"""{WIKIFY_BEGIN}
## Code wikis (wikify)
Grounded internals wikis for ingested repositories live under `{w}/<slug>/` (generated by
wikify-repo: every claim cites a real symbol, checked by a linter); `{w}/index.md` lists them. For
questions about a repository's internals, retrieve from its wiki instead of reading source: read
`{w}/<slug>/overview.md` first and follow its map; `grep` for the relevant `concepts/` (mechanism)
or `catalog/` (per-symbol) page and read only that section; cite the catalog anchor
`catalog/<module>.md#<Symbol>` and follow its source link only when you need the exact line; do not
bulk-read pages or guess. Do not hand-edit `catalog/`, `index.md` or `log.md` inside a silo; they
are regenerated by `wikify finalize`. To ingest or update a repository, invoke the
`wikify-ingest-repo` skill: "wikify <repo url or path>".
{WIKIFY_END}"""


def _instruction_targets(project: Path, instructions: str | None) -> list[Path]:
    """Where the block goes in a host project: an explicit list; else SCHEMA.md when the
    project routes its agent files through one; else the agent files that exist; else
    CLAUDE.md + AGENTS.md."""
    if instructions:
        return [project / x.strip() for x in instructions.split(",") if x.strip()]
    if (project / "SCHEMA.md").exists():
        return [project / "SCHEMA.md"]
    present = [project / f for f in ("CLAUDE.md", "AGENTS.md", "GEMINI.md") if (project / f).exists()]
    return present or [project / "CLAUDE.md", project / "AGENTS.md"]
...
def setup(
    ...
    instructions: str = typer.Option(None, help="With --project: comma-separated files to inject the "
    ...
    Replaces scripts/setup-vendor.sh and scripts/install-skill.sh. Idempotent. With
    ...
    if project is not None:
        ...
        if not no_instructions:
            ...
            for f in _instruction_targets(project, instructions):
                typer.echo(f"{f.relative_to(project)}: {inject_instructions(f, block)} wikify block")
