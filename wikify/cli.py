...
- **Symbols:** the index `{w}/catalog/symbols.tsv` (`symbols/*.tsv` when sharded; columns anchor, path,
  ...
  whole, and mind the search tool's line cap: `grep -c` first, narrow a hub's callers by directory
  (`cut -f2 | cut -d/ -f1-2 | sort | uniq -c`), end every listing with `| head -20`.
  `grep -P '^<path>#<Symbol>\\t'` gives one row or, on edges, its callers;
  `grep -P '\\t<path>#<Symbol>$'` on edges gives what it calls;
  `grep -P '[#.]<Name>\\t' | sort -t$'\\t' -k5 -nr | head -20` finds a bare name,
...
- Never bulk-read pages or hand-edit `{w}/catalog/`, `{w}/index.md`, `{w}/log.md`, `{w}/changes/` or
  `area:auto` blocks (regenerated by `wikify finalize`). To build or update the wiki, invoke the
  `wikify-ingest-repo` skill from the repo root ("wikify this repo"); `wikify plan` previews what is
  stale. Config: `wikify.md`.
{WIKIFY_END}"""
