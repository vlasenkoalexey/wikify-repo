...
- **Multi-language** (`wikify/languages.py`): grounding is SCIP (language-neutral), so a language
  ...
  and **auto-installs** a missing indexer, announcing the command it runs (opt out with
  `--no-install-indexers`, which prints the guidance and skips that language). Each writes
